...
export const generateMetadata = createMetadataGenerator('', 'page_generic', store.api)
...
export default async function getGenericPage({
  ...
  const result = await store.api.find<GenericPageData>('page_generic', preview, (query) => {
